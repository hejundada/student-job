  ...
  <div class="Resume">
    ...
    <div class="Resume-content">
      <div class="Resume-content-form">
        ...
        <div class="form-text">
          <label>职位名称:</label>
          <input type="text" v-model="modifydata.jobname">
        </div>
        <div class="form-text">
          <label>企业名称:</label>
        ...
        <div class="form-text">
          <label>薪资:</label>
          <input type="text" v-model="modifydata.money">
        </div>
        <div class="form-text">
          <label>招聘类型:</label>
          <!-- <select v-model="modifydata.jobtype">
                                <option value="1">销售类</option>
                                <option value="2">技术类</option>
                                <option value="3">互联网类</option>
                                <option value="4">活动类</option>
          </select>-->
        ...
        <div class="form-text">
          <label>职位要求:</label>
          <textarea type="text" v-model="modifydata.need"></textarea>
        </div>
...
export default {
  ...
  methods: {
    infor(){
        ...
        method: "post",
        url:
          "http://114.115.147.52:7076/employType/allEmployType"
      })
        .then(response => {
          //content title
        //   console.log(response.data)
          if (response.data.code == 100) {
            this.raido = response.data.data;
          }
        })
        .catch(err => {
          console.log(err);
    ...
    begindata() {
      axios({
        method: "post",
        url:
          "http://114.115.147.52:7076/employ/addEmploy" +
          "?companyId=" +
          getCookie("CompanyID") +
          "&title=" +
          this.modifydata.jobname +
          "&companyName=" +
          getCookie("companyName") +
          "&salary=" +
          this.modifydata.money +
          "&type=" +
         this.id +
          "&content=" +
          this.modifydata.need
      })
        .then(response => {
          //content title
          // console.log(response.data)
...
.Resume-content-form .form-text select {
  width: 3.8rem;
  height: 0.4rem;
  line-height: 0.4rem;
  border: 1px solid #dcdcdc;
