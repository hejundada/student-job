<template>
  <div class="student-box" v-bind:style="{height:myHeight}">
    <div class="box-infor">
      <div class="header-text">用人单位列表</div>
      <div class="table-box">
        <table id="tableSort" style="table-layout:fixed;">
          ...
          <tbody class="table-lists">
            ...
            >
              ...
              <td class="sequence">{{item.companyName}}</td>
              <td class="sequence">
                <a class="amend-infor" @click="amend(item.id,item.companyName)">修改</a>
        ...
        <div class="sub-popup-con">
          ...
          <div class="amend-infor2">
            <div class="amend-infor-list">
              <label>公司名称</label>
              <el-input class="el-input2" v-model="name" placeholder="请输入公司名"></el-input>
            </div>
          </div>
  ...
  methods: {
    ...
    confirm() {
      if(this.name == ''){
        ...
        axios({
          method: "post",
          url: "http://114.115.147.52:7076/company/updateCompany"+"?id="+getCookie("cpId")+"&companyName="+this.name,
          headers: {
    ...
    del(i){
       axios({
          method: "post",
          url: "http://114.115.147.52:7076/company/deleteCompany"+"?id="+i,
          headers: {
            "Content-Type": "application/json;charset=UTF-8"
          }
        })
    ...
    employers(){
      axios({
          method: "post",
          url: "http://114.115.147.52:7076/company/getCompanys",
          headers: {
            "Content-Type": "application/json;charset=UTF-8"
          }
        })
